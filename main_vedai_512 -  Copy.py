# 1: car, 2:trucks, 4: tractors, 5: camping cars, 6,23:boats 7,8,10,12:others, 9: vans  11: pickup, 31: plane
...
def update_annotations(filename):
		data = pd.read_csv('annotations512/' + filename, sep=' ', index_col=None, header=None, names=['x_center', 'y_center', 'orientation', 'class', 'is_contained', 'is_occluded', 'corner1_x', 'corner2_x', 'corner3_x', 'corner4_x', 'corner1_y', 'corner2_y', 'corner3_y', 'corner4_y'])
		...
		width = ET.SubElement(size, 'width').text = '512'
		height = ET.SubElement(size, 'height').text = '512'
		...
		global COUNT_TRUCK
		global COUNT_PICKUP
		global COUNT_TRACTOR
		global COUNT_VAN
		global COUNT_CAMPCAR
		global COUNT_PLANE
		global COUNT_OTHER
		global COUNT_BOAT
		for row in range(len(data)):
			...
			if data.iloc[row]['class'] == 1:
				...
				COUNT_CAMPCAR += 1
			elif data.iloc[row]['class'] == 6 or data.iloc[row]['class'] == 23:
				name = ET.SubElement(object, 'name').text = 'boat'
				COUNT_BOAT += 1
			elif data.iloc[row]['class'] == 7 or data.iloc[row]['class'] == 8 or data.iloc[row]['class'] == 10 or data.iloc[row]['class'] == 12:
			...
			try:
				...
				tree.write("image_512/{0}.xml".format(imagename))
...
if __name__ == '__main__':
	list = os.listdir('annotations512/')
	...
	print("Count Tractor:{0}".format(COUNT_TRACTOR))
	print("Count Van:{0}".format(COUNT_VAN))
	print("Count Camping car:{0}".format(COUNT_CAMPCAR))
	print("Count Plane:{0}".format(COUNT_PLANE))
	print("Count Boat:{0}".format(COUNT_BOAT))
	print("Count Other:{0}".format(COUNT_OTHER))
